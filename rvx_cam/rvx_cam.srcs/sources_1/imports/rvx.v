...
module rvx #(
  ...
  assign device_start_address [32*D5_CAMERA   +: 32]  = 32'h8004_0000;
  assign device_region_size   [32*D5_CAMERA   +: 32]  = 16;
  ...
  // ======================================================================
  // HARDWARE ACCELERATOR BUS LOGIC (CAMERA & SOBEL)
  // ======================================================================
  ...
  // 1. Memory-Mapped Write Logic
  always @(posedge clock) begin
      if (reset) begin
          cam_enable_reg    <= 1'b1; // Default: Camera ON
          filter_enable_reg <= 1'b0; // Default: Raw View
          resolution_reg    <= 1'b0; // Default: 640x480
      end else if (device_write_request[D5_CAMERA] && device_write_strobe[0]) begin
          if (device_rw_address[3:0] == 4'h0) begin
              cam_enable_reg    <= device_write_data[0];
              filter_enable_reg <= device_write_data[1];
              resolution_reg    <= device_write_data[2];
          end
      end
  end

  // 2. Memory-Mapped Read Logic (FIXED: Registered to break LUT loop)
  reg [31:0] camera_read_data_reg;
  
  always @(posedge clock) begin
      if (reset) begin
          camera_read_data_reg <= 32'b0;
      end else if (device_read_request[D5_CAMERA]) begin
          // Only output the status word if the CPU is reading address 0x80040000
          if (device_rw_address[3:0] == 4'h0) begin
              camera_read_data_reg <= {29'b0, resolution_reg, filter_enable_reg, cam_enable_reg};
          end else begin
              camera_read_data_reg <= 32'b0;
          end
      end
  ...
  // Drive the bus with the registered data
  assign device_read_data[32*D5_CAMERA +: 32] = camera_read_data_reg;

  // 3. Bus Handshaking (Registered Response)
  reg cam_write_resp;
  reg cam_read_resp;
  ...
  always @(posedge clock) begin
      if (reset) begin
          ...
          // Delay the acknowledgement by 1 clock cycle
          cam_write_resp <= device_write_request[D5_CAMERA];
          cam_read_resp  <= device_read_request[D5_CAMERA];
      end
  end
  
  assign device_write_response[D5_CAMERA] = cam_write_resp;
  assign device_read_response[D5_CAMERA]  = cam_read_resp;

  // 4. Instantiate the Camera Pipeline
  camera_top accel_inst (
      .sio_c(sio_c),
      .sio_d(sio_d),
      .reset(cam_reset),
      .pwdn(cam_pwdn),
      ...
      .clk(clock), 
      .rst(reset),
      .cam_en(cam_enable_reg),
      .filter_en(filter_enable_reg),
      .res_sel(resolution_reg)
  );
